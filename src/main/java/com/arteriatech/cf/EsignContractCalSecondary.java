...
public class EsignContractCalSecondary extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload="",wsURL="",userName="",passWord="",userpass="",esignContractEndPoint="",esignContractres="";
		boolean debug=false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					esignContractEndPoint = properties.getProperty("eSignContractCALSecondary");
					wsURL = wsURL.concat(esignContractEndPoint);
					if (debug) {
						response.getWriter().println("EsignContractCalSecondary cpi  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						esignContractres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(esignContractres);
