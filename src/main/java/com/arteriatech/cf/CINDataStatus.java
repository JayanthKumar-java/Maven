...
public class CINDataStatus extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", idpUserDetailsEndPoint = "",
				idpUserDetailsres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if(inpJsonPayLoad.get("FetchbyCIN").getAsJsonObject().has("debug")&&!inpJsonPayLoad.get("FetchbyCIN").getAsJsonObject().get("debug").isJsonNull()&&inpJsonPayLoad.get("FetchbyCIN").getAsJsonObject().get("debug").getAsString().equalsIgnoreCase("true")){
					debug = true;
				}
				if (debug) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					idpUserDetailsEndPoint = properties.getProperty("CINDataStatus");
					wsURL = wsURL.concat(idpUserDetailsEndPoint);
					if (debug) {
						response.getWriter().println("CINDataStatus cpi  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						idpUserDetailsres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(idpUserDetailsres);
						responseJson.addProperty("Remarks", "");
						if (debug)
							response.getWriter().println("responseJson: " + responseJson.toString());
