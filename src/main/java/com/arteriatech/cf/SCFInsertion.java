...
public class SCFInsertion extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", scfInsertionEndPoint = "",
				scfInsertionres = "";
		boolean debug = false;
		...
		try {
			...
			//inputPayload=request.getParameter("FetchbyCIN");
			
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				inpJsonPayLoad = (JsonObject) parser.parse(inputPayload);
				if(inpJsonPayLoad.has("debug") && inpJsonPayLoad.get("debug").getAsString().equalsIgnoreCase("true")){
					debug = true;
				...
				//cpiPayload.accumulate("Root",inpJsonPayLoad.get("Root").getAsJsonObject());
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					scfInsertionEndPoint = properties.getProperty("SCFInsertion");
					wsURL = wsURL.concat(scfInsertionEndPoint);
					if (debug) {
						response.getWriter().println("SCFInsertion cpi  Url: " + wsURL);
					}
					if(debug){
						response.getWriter().println(" CPI Input Payload "+inpJsonPayLoad);
					}
					...
					if ((resCode / 100) == 2) {
						...
						scfInsertionres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(scfInsertionres);
