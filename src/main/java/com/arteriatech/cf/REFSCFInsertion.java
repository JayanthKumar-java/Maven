...
public class REFSCFInsertion extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload="",wsURL="",userName="",passWord="",userpass="",refScfInsertionEndPoint="",refScfInsertionres="";
		boolean debug=false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					refScfInsertionEndPoint = properties.getProperty("REFSCFInsertion");
					wsURL = wsURL.concat(refScfInsertionEndPoint);
					if (debug) {
						response.getWriter().println("REFSCFInsertion cpi  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						refScfInsertionres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(refScfInsertionres);
