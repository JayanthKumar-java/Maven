...
public class GETDownloadedROCFiles extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", getDownRocFileEndPoint = "",
				getDownRocFileres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					getDownRocFileEndPoint = properties.getProperty("GETDownloadedROCFiles");
					wsURL = wsURL.concat(getDownRocFileEndPoint);
					if (debug) {
						response.getWriter().println("GETDownloadedROCFiles cpi  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						getDownRocFileres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(getDownRocFileres);
