...
public class BuyerLimitMISProxy extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", buyerLimtProxyEndPoint = "",
				buyerLimitPrpxyRes = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					buyerLimtProxyEndPoint = properties.getProperty("BuyerLimitMISProxy");
					wsURL = wsURL.concat(buyerLimtProxyEndPoint);
					if (debug) {
						response.getWriter().println("BuyerLimitMISProxy cpi  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						buyerLimitPrpxyRes = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(buyerLimitPrpxyRes);
