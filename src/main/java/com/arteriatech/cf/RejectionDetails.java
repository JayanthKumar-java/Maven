...
public class RejectionDetails extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String userName = "", passWord = "", userpass = "", aggregatorId = "", RejectionDetailsEndPoint = "",
				rejectionDestUrl = "", rejDetResponse = "";
		try {
			...
			if (!requestedPayload.equals("") && requestedPayload.trim().length() > 0) {
				...
				if (inputPayload.has("BPID") && !inputPayload.get("BPID").getAsString().equals("")
						&& inputPayload.get("BPID").getAsString() != null) {
					...
					if (mandatoryCheckPass) {
						...
						// DestinationConfiguration rejectionConfig = configuration.getConfiguration(destName);
						DestinationOptions options = DestinationOptions.builder().augmentBuilder(ScpCfDestinationOptionsAugmenter
								.augmenter().tokenExchangeStrategy(ScpCfDestinationTokenExchangeStrategy.LOOKUP_ONLY)).build();
						Try<Destination> destinationAccessor = DestinationAccessor.getLoader()
								.tryGetDestination(destName, options);
						Destination rejectionConfig = destinationAccessor.get();

						wsURL = rejectionConfig.get("URL").get().toString();
						if (debug) {
							response.getWriter().println("wsURL: " + wsURL);
						}
						if (wsURL != null && wsURL.trim().length() > 0) {
							userName = rejectionConfig.get("User").get().toString();
							passWord = rejectionConfig.get("Password").get().toString();
							userpass = userName + ":" + passWord;
							aggregatorId = rejectionConfig.get("AggregatorID").get().toString();
							// RejectionDetails
							RejectionDetailsEndPoint = properties.getProperty("RejectionDetails");
							rejectionDestUrl = wsURL.concat(RejectionDetailsEndPoint);
							if (debug) {
								response.getWriter().println("aggregatorId: " + aggregatorId);
								response.getWriter().println("Rejection Details Url: " + rejectionDestUrl);
							}
							...
							URL url = new URL(rejectionDestUrl);
							...
							if ((responseCode / 100) == 2) {
								...
								rejDetResponse = sb.toString();
								JsonParser leiResponseParser = new JsonParser();
								JsonObject responseJson = (JsonObject) leiResponseParser.parse(rejDetResponse);
