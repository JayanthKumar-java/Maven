...
public class EsignContractCall extends HttpServlet {
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					esignContractCalEndPoint = properties.getProperty("eSignContractCAL");
					wsURL = wsURL.concat(esignContractCalEndPoint);
					if (debug) {
						response.getWriter().println("EsignContractCall cpi  Url: " + wsURL);
					}
