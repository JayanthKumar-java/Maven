...
public class SCFeSignContractUpdate extends HttpServlet{
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload="",wsURL="",userName="",passWord="",userpass="",scfEsinCntEndPoint="",scfEsinCntres="";
		boolean debug=false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					scfEsinCntEndPoint = properties.getProperty("SCFeSignContractUpdate");
					wsURL = wsURL.concat(scfEsinCntEndPoint);
					if (debug) {
						response.getWriter().println("SCFEsignContractUpdate cpi  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						scfEsinCntres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(scfEsinCntres);
