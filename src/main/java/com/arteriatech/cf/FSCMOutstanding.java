...
/**
 * Servlet implementation class FSCMOutstanding
 */
@WebServlet("/FSCMOutstanding")
public class FSCMOutstanding extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final String CPI_CONNECTION_DESTINATION =  "CPIConnect";
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FSCMOutstanding() {
        super();
...
//		DestinationConfiguration destConfiguration = null;
		...
		try{
			...
			payloadRequest = request.getParameter("FSCMOutstanding");//getGetBody(request, response);
			if(debug)
...
//						DestinationConfiguration cpiDestConfig = configuration.getConfiguration(destName);
						...
						if(wsURL != null && wsURL.trim().length() > 0){
							...
							cpiEndPoint = properties.getProperty("FSCMAnchorOutstanding");
