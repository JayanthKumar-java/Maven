...
public class IDPUserDetails extends HttpServlet{
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					idpUserDetailsEndPoint = properties.getProperty("IDPUserDetails");
					wsURL = wsURL.concat(idpUserDetailsEndPoint);
					if (debug) {
						response.getWriter().println("IDPUserDetails cpi  Url: " + wsURL);
					}
