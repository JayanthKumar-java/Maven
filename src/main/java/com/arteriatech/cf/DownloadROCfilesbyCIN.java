...
public class DownloadROCfilesbyCIN extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", dwnRocFilByCINEndPoint = "",
				dwnRocFilByCINres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					dwnRocFilByCINEndPoint = properties.getProperty("DownloadROCfilesbyCIN");
					wsURL = wsURL.concat(dwnRocFilByCINEndPoint);
					if (debug) {
						response.getWriter().println("DownloadROCfilesbyCIN cpi  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						dwnRocFilByCINres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(dwnRocFilByCINres);
