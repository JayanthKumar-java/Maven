...
public class FetchbyCIN extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", fetchbyCINEndPoint = "",
				fetchbyCINres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					fetchbyCINEndPoint = properties.getProperty("FetchbyCIN");
					wsURL = wsURL.concat(fetchbyCINEndPoint);
					if (debug) {
						response.getWriter().println("FetchbyCIN cpi  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						fetchbyCINres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(fetchbyCINres);
