...
public class Secondary extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", secondaryEndPoint = "",
				secondaryres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					secondaryEndPoint = properties.getProperty("Secondary");
					wsURL = wsURL.concat(secondaryEndPoint);
					if (debug) {
						response.getWriter().println("Secondary cpi  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						secondaryres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(secondaryres);
