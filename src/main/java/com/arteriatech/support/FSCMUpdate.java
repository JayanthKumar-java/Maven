...
public class FSCMUpdate extends HttpServlet{
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		String inputPayload="",wsURL="",userName="",passWord="",userpass="",fscmUpdateEndPoint="",FSCMUpdateres="";
		boolean debug=false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					fscmUpdateEndPoint = properties.getProperty("RFCCallBackStatusUpdate");//RFCCallBackStatusUpdateDINVONUPD
					wsURL = wsURL.concat(fscmUpdateEndPoint);
					if (debug) {
						response.getWriter().println("FSCMUpdate  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						FSCMUpdateres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(FSCMUpdateres);
