...
public class GetSCFOffer extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload="",wsURL="",userName="",passWord="",userpass="",getSCFOfferEndPoint="",getSCFOfferres="";
		boolean debug=false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (debug) {
					...
					getSCFOfferEndPoint = properties.getProperty("GetSCFOffer");
					wsURL = wsURL.concat(getSCFOfferEndPoint);
					if (debug) {
						response.getWriter().println("GetSCFOffer  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2 || (resCode / 100)==3) {
						...
						getSCFOfferres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(getSCFOfferres);
						...
						responseJson.addProperty("Message", getSCFOfferres);
						responseJson.addProperty("Remarks", "");
