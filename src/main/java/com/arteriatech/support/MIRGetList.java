...
public class MIRGetList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", mIRGetListEndPoint = "",
				mIRGetListEndPointres = "";
		boolean debug = false;
		...
		try {
			...
			inputPayload=request.getParameter("MIRGetList");
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					mIRGetListEndPoint = properties.getProperty("FBL1N");
					wsURL = wsURL.concat(mIRGetListEndPoint);
					if (debug) {
						response.getWriter().println("MIRGetList Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2 ||(resCode / 100)==3 ) {
						...
						mIRGetListEndPointres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(mIRGetListEndPointres);
					...
					result.addProperty("Message", "Empty Input Payload Received in the request");
