...
public class CustomerOutstandings extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", custosgtlstEndPoint = "",
				custosgtlstres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					custosgtlstEndPoint = properties.getProperty("FBL5N");
					wsURL = wsURL.concat(custosgtlstEndPoint);
					if (debug) {
						response.getWriter().println("CustomerOutstandings Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						custosgtlstres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(custosgtlstres);
					...
					result.addProperty("Message", "Empty Input payload ");
