...
public class RegistrationStatus extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload="",wsURL="",userName="",passWord="",userpass="",registrationStatusendPoint="",registrationStatusRes="";
		boolean debug=false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					registrationStatusendPoint = properties.getProperty("UserRegistrationStatus");
					wsURL = wsURL.concat(registrationStatusendPoint);
					if (debug) {
					...
					if ((resCode / 100) == 2 || (resCode/100)==3) {
						...
						registrationStatusRes = sb.toString();
						...
						if (debug)
							response.getWriter().println("responseJson: " + registrationStatusRes);
						response.getWriter().println(registrationStatusRes);
