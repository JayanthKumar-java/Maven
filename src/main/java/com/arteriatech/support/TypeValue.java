...
public class TypeValue extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", typeValuePoint = "",
				typeValueres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (debug) {
					...
					typeValuePoint = properties.getProperty("TypeValueGet");
					wsURL = wsURL.concat(typeValuePoint);
					if (debug) {
						response.getWriter().println("TypeValueGet Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2 || (resCode / 100) == 3) {
						...
						typeValueres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(typeValueres);
