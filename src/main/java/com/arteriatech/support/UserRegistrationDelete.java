...
public class UserRegistrationDelete extends HttpServlet{
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "",
				userRegistrationDelete = "", userRegistrationDeleteres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (debug) {
					...
					userRegistrationDelete = properties.getProperty("UserRegistrationDelete");
					wsURL = wsURL.concat(userRegistrationDelete);
					if (debug) {
						response.getWriter().println("UserRegistration Delete   Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2 || (resCode / 100)==3) {
						...
						userRegistrationDeleteres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(userRegistrationDeleteres);
