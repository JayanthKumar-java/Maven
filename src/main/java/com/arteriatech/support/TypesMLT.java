...
public class TypesMLT extends HttpServlet {	
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", typesMLTEndPoint = "",
				typesMLTres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				typesMLTEndPoint = properties.getProperty("TypesMLTGet");
				wsURL = wsURL.concat(typesMLTEndPoint);
				if (debug) {
					response.getWriter().println("TypesMLTGet Url: " + wsURL);
				}
				...
				if ((resCode / 100) == 2 || (resCode / 100) == 3) {
					...
					typesMLTres = sb.toString();
					JsonParser jsonParser = new JsonParser();
					JsonObject responseJson = (JsonObject) jsonParser.parse(typesMLTres);
