...
public class SCFEligibility extends HttpServlet {
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", SCFEligibilityEndPoint = "",
				SCFEligibilityres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				String validateInputPayload = validateInputPayload(inpJsonPayLoad);
				if (validateInputPayload.equalsIgnoreCase("")) {
					...
					SCFEligibilityEndPoint = properties.getProperty("SCF1Reset");
					wsURL = wsURL.concat(SCFEligibilityEndPoint);
					if (debug) {
					...
					if ((resCode / 100) == 2) {
						...
						SCFEligibilityres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(SCFEligibilityres);
						...
						result.addProperty("Message", SCFEligibilityres);
					...
					result.addProperty("Status", "000002");
					result.addProperty("ErrorCode", "/ARTEC/J001");
					result.addProperty("Message", validateInputPayload);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", SCFEligibilityEndPoint = "",
				SCFEligibilityres = "";
		JsonObject cpiPayload = new JsonObject();
		boolean debug = false;
		JsonObject inpJsonPayLoad = new JsonObject();
		Properties properties = new Properties();
		String aggId = "", cptype = "", cpGuid = "";
		properties.load(getServletContext().getResourceAsStream("/Resources/KeyProperties.properties"));
		try {
			// inputPayload = commonUtils.getGetBody(request, response);
			inputPayload = request.getParameter("SCFEligibility");
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				inpJsonPayLoad = (JsonObject) parser.parse(inputPayload);
				if (inpJsonPayLoad.has("debug") && !inpJsonPayLoad.get("debug").isJsonNull()
						&& inpJsonPayLoad.get("debug").getAsString().equalsIgnoreCase("true")) {
				...
				if (debug) {
					...
					passWord = cpiConfig.get("Password").get().toString();
					userpass = userName + ":" + passWord;
					SCFEligibilityEndPoint = properties.getProperty("SCF1_GET");
					wsURL = wsURL.concat(SCFEligibilityEndPoint);
					if (debug) {
						response.getWriter().println("SCFEligibility Get Url: " + wsURL);
					}
					if (inpJsonPayLoad.has("AggregatorID") && !inpJsonPayLoad.get("AggregatorID").isJsonNull()) {
						aggId = inpJsonPayLoad.get("AggregatorID").getAsString();
					}
					if (inpJsonPayLoad.has("CPTypeID") && !inpJsonPayLoad.get("CPTypeID").isJsonNull()) {
						cptype = inpJsonPayLoad.get("CPTypeID").getAsString();
					}
					if (inpJsonPayLoad.has("CPGUID") && !inpJsonPayLoad.get("CPGUID").isJsonNull()) {
						cpGuid = inpJsonPayLoad.get("CPGUID").getAsString();
					}
					cpiPayload.addProperty("AGGRID", aggId);
					cpiPayload.addProperty("CPType", cptype);
					cpiPayload.addProperty("CPGuid", cpGuid);
					if (debug) {
						response.getWriter().println("CPI Input Json Payload " + cpiPayload);
					}
					URL url = new URL(wsURL);
					HttpsURLConnection urlConnection = (HttpsURLConnection) url.openConnection();
					byte[] bytes = cpiPayload.toString().getBytes("UTF-8");
