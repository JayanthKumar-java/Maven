...
public class RenewalSCFReset extends HttpServlet{
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "",
				renewalSCF1Reset = "", renewalSCF1Resetres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				renewalSCF1Reset = properties.getProperty("RenewalSCFReset");
				wsURL = wsURL.concat(renewalSCF1Reset);
				if (debug) {
					response.getWriter().println("RenewalSCFReset   Url: " + wsURL);
				}
