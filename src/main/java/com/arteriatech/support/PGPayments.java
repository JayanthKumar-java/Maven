...
public class PGPayments extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", PGPaymentsEndPoint = "",
				PGPaymentsres = "";
		boolean debug = false;
		...
		try {
			...
			inputPayload=request.getParameter("PGPayments");
			if(debug){
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					PGPaymentsEndPoint = properties.getProperty("PGPayments");
					wsURL = wsURL.concat(PGPaymentsEndPoint);
					if (debug) {
						response.getWriter().println("PGPayments Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2 || (resCode / 100) == 3) {
						...
						PGPaymentsres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(PGPaymentsres);
