...
public class UserRegistrationGet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", userRegistrationGet = "",
				userRegistrationRes = "";
		boolean debug = false;
		...
		String errorMessage = "";
		try {
			inputPayload = request.getParameter("UserRegistrationGet");
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (errorMessage.equalsIgnoreCase("")) {
					...
					userRegistrationGet = properties.getProperty("UserRegistrationGet");
					wsURL = wsURL.concat(userRegistrationGet);
					if (debug) {
					...
					if ((resCode / 100) == 2 || (resCode / 100) == 3) {
						...
						JsonObject responseJson = (JsonObject) jsonParser.parse(userRegistrationRes);
				...
				result.addProperty("Status", "000002");
				result.addProperty("ErrorCode", "/ARTEC/J001");
				result.addProperty("Message", "Empty Input Payload Received in the Paylaod");
				response.getWriter().println(new Gson().toJson(result));
			}
