...
public class UserRegStatus extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "",
				userRegStatusEndPoint = "", userRegStatusRes = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					userRegStatusEndPoint = properties.getProperty("UserRegStatus");
					wsURL = wsURL.concat(userRegStatusEndPoint);
					if (debug) {
						response.getWriter().println("UserRegStatus Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						userRegStatusRes = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(userRegStatusRes);
					...
					result.addProperty("Message", "CPI URL is Empty");
