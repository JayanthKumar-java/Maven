...
public class SCFDiscountedInvoices extends HttpServlet {
	/**
	 * 
	 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "",
				SCFDiscountedInvoicesEndPoint = "", SCFDiscountedInvoicesres = "";
		boolean debug = false;
		...
		try {
			...
			inputPayload=request.getParameter("SCF2");
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				String errorMessage = validateinputPayload(inpJsonPayLoad);
				if (errorMessage.equalsIgnoreCase("")) {
					...
					SCFDiscountedInvoicesEndPoint = properties.getProperty("SCF2Get");
					wsURL = wsURL.concat(SCFDiscountedInvoicesEndPoint);
					if (debug) {
						response.getWriter().println("SCFDiscountedInvoices Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2 || (resCode / 100) == 3) {
						...
						SCFDiscountedInvoicesres = sb.toString();

						JSONObject jsonRes = new JSONObject(SCFDiscountedInvoicesres);
		...
		try {
			...
			/*if (inpJsonPayLoad.has("CPGUID")) {
				if (inpJsonPayLoad.get("CPGUID").isJsonNull()
						|| inpJsonPayLoad.get("CPGUID").getAsString().equalsIgnoreCase("")) {
					if (errorMessage.equalsIgnoreCase("")) {
			...
			if (inpJsonPayLoad.has("CompanyCode")) {
				if (inpJsonPayLoad.get("CompanyCode").isJsonNull()
						|| inpJsonPayLoad.get("CompanyCode").getAsString().equalsIgnoreCase("")) {
					if (errorMessage.equalsIgnoreCase("")) {
						errorMessage = "CompanyCode Missing in the input payload";
					} else {
						errorMessage = errorMessage + ",CompanyCode Missing in the input payload";
					}
				}
			} else {
				if (errorMessage.equalsIgnoreCase("")) {
