...
public class CFRegistration extends HttpServlet {
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload="",wsURL="",userName="",passWord="",userpass="",CFRegistrationEndPoint="",CFRegistrationres="";
		boolean debug=false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					CFRegistrationEndPoint = properties.getProperty("CFRegistrationDelete");
					wsURL = wsURL.concat(CFRegistrationEndPoint);
					if (debug) {
						response.getWriter().println("CFRegistrationDelete  Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						CFRegistrationres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(CFRegistrationres);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//debug=true;
		try {
			inputPayload=request.getParameter("CFRegistration");
			if(debug){
				response.getWriter().println(inputPayload);
			}
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				inpJsonPayLoad = (JsonObject) parser.parse(inputPayload);
				if (inpJsonPayLoad.has("debug") && !inpJsonPayLoad.get("debug").isJsonNull()
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					passWord = cpiConfig.get("Password").get().toString();
					userpass = userName + ":" + passWord;
					CFRegistrationEndPoint = properties.getProperty("CFRegistrationGET");
					wsURL = wsURL.concat(CFRegistrationEndPoint);
					if (debug) {
						response.getWriter().println("CFRegistrationGet  Url: " + wsURL);
					}
					URL url = new URL(wsURL);
					HttpsURLConnection urlConnection = (HttpsURLConnection) url.openConnection();
					byte[] bytes = inpJsonPayLoad.toString().getBytes("UTF-8");
					...
					if ((resCode / 100) == 2) {
						...
						responseJson.addProperty("Status", "000002");
						responseJson.addProperty("ErrorCode", "/ARTEC/J001");
