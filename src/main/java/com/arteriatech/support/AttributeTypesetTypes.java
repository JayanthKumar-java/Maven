...
public class AttributeTypesetTypes extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", attributeTypeSetUrl = "",
				attributeTypeSetRes = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (debug) {
					...
					attributeTypeSetUrl = properties.getProperty("AttributeTypesetTypes");
					wsURL = wsURL.concat(attributeTypeSetUrl);
					if (debug) {
						response.getWriter().println("AttributeTypesetTypes Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2 || (resCode / 100) == 3) {
						...
						attributeTypeSetRes = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(attributeTypeSetRes);
