...
public class RenewalSCFGet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "",renewalSCFUrl = "",
				renewalSCFres = "";
		boolean debug = false;
		...
		String errorMessage="";
		try {
			inputPayload = request.getParameter("RenewalSCF");
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				renewalSCFUrl = properties.getProperty("RenewalSCFGet");
				wsURL = wsURL.concat(renewalSCFUrl);
				if (debug) {
				...
				if ((resCode / 100) == 2 || (resCode / 100) == 3) {
					...
					renewalSCFres = sb.toString();
					JsonParser jsonParser = new JsonParser();
					JsonObject responseJson = (JsonObject) jsonParser.parse(renewalSCFres);
