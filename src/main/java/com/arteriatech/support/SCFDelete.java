...
public class SCFDelete extends HttpServlet{
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", scfDeleteEndpoint = "",
				scfDeleteRes = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (validatedInpuPayload.equalsIgnoreCase("")) {
					...
					scfDeleteEndpoint = properties.getProperty("SCFDelete");
					wsURL = wsURL.concat(scfDeleteEndpoint);
					if (debug) {
					...
					if ((resCode / 100) == 2) {
						...
						scfDeleteRes = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(scfDeleteRes);
						...
						result.addProperty("Message", scfDeleteRes);
