...
public class EsignContractUpdate extends HttpServlet {
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload="",wsURL="",userName="",passWord="",userpass="",esignCntUpdEndPoint="",esignCntUpdres="";
		boolean debug=false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					esignCntUpdEndPoint = properties.getProperty("EsignCntUpd");
					wsURL = wsURL.concat(esignCntUpdEndPoint);
					if (debug) {
					...
					if ((resCode / 100) == 2) {
						...
						esignCntUpdres = sb.toString();
						JsonParser leiResponseParser = new JsonParser();
						JsonObject responseJson = (JsonObject) leiResponseParser.parse(esignCntUpdres);
						...
						JsonObject responseJson = (JsonObject) jsonParser.parse(esignCntUpdres);
						responseJson.addProperty("Remarks", "");
						if (debug)
							response.getWriter().println("responseJson: " + responseJson);
						response.getWriter().println(responseJson);
