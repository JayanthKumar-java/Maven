...
public class RenewalSCF1Get extends HttpServlet{
	/**
	 * 
	 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "",renewalSCF1Url = "",
				renewalSCF1res = "";
		boolean debug = false;
		...
		String errorMessage="";
		try {
			inputPayload = request.getParameter("RenewalSCF1");
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				renewalSCF1Url = properties.getProperty("RenewalSCF1Get");
				wsURL = wsURL.concat(renewalSCF1Url);
				if (debug) {
				...
				if ((resCode / 100) == 2 || (resCode / 100) == 3) {
					...
					renewalSCF1res = sb.toString();
					JsonParser jsonParser = new JsonParser();
					JsonObject responseJson = (JsonObject) jsonParser.parse(renewalSCF1res);
