...
public class MISPublishOnDemand extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "", misPublishOnDemandEndPoint = "",
				misPublishOnDemandres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					misPublishOnDemandEndPoint = properties.getProperty("MISPublishOnDemand");
					wsURL = wsURL.concat(misPublishOnDemandEndPoint);
					if (debug) {
						response.getWriter().println("MISPublishOnDemand Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2) {
						...
						misPublishOnDemandres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(misPublishOnDemandres);
