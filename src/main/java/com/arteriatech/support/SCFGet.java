...
public class SCFGet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "",scfGetEndpoint = "",
				scfGetres = "";
		boolean debug = false;
		...
		String errorMessage="";
		try {
			inputPayload = request.getParameter("SCFGet");
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				scfGetEndpoint = properties.getProperty("SCFGet");
				wsURL = wsURL.concat(scfGetEndpoint);
				if (debug) {
				...
				if ((resCode / 100) == 2 || (resCode / 100) == 3) {
					...
					scfGetres = sb.toString();
					JsonParser jsonParser = new JsonParser();
					JsonObject responseJson = (JsonObject) jsonParser.parse(scfGetres);
