...
public class InvoiceDetails extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload="",wsURL="",userName="",passWord="",userpass="",InvoiceDetailsendPoint="",InvoiceDetailsRes="";
		boolean debug=false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					InvoiceDetailsendPoint = properties.getProperty("FSCMAnchorOutstanding");
					wsURL = wsURL.concat(InvoiceDetailsendPoint);
					if (debug) {
						response.getWriter().println("esignCntUpdendPoint  Url: " + wsURL);
					}
					...
					if (debug) {
						...
						response.getWriter().println("Response Message: " + urlConnection.getResponseMessage());
					}
					if ((resCode / 100) == 2 || (resCode/100)==3) {
						...
						InvoiceDetailsRes = sb.toString();
						/*JsonParser leiResponseParser = new JsonParser();
						JsonObject responseJson = (JsonObject) leiResponseParser.parse(registrationStatusRes);
						...
						JsonObject responseJson = (JsonObject) jsonParser.parse(registrationStatusRes);
						responseJson.addProperty("Remarks", "");*/
						if (debug)
							response.getWriter().println("responseJson: " + InvoiceDetailsRes);
						response.getWriter().println(InvoiceDetailsRes);

					}
				} else {
					if (debug)
						response.getWriter().println("WSURL is Empty");
					JsonObject result = new JsonObject();
					result.addProperty("Status", "000002");
					result.addProperty("ErrorCode", "/ARTEC/J001");
					result.addProperty("Message", "wsURL is Empty");
