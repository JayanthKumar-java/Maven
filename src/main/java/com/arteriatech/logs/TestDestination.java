 ...
 */
@WebServlet("/TestDestination")
public class TestDestination extends HttpServlet {
...
//	int RequestCount = 0;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        response.getWriter().println("1. doGet function starts");
		response.getWriter().println("2 Inside doGet for GetLoginID.getCurrentToken.getJwt: " +AuthTokenAccessor.getCurrentToken().getJwt());
		response.getWriter().println("3 Inside doGet for GetLoginID.getCurrentPrincipal: "+PrincipalAccessor.getCurrentPrincipal());
		response.getWriter().println("4 Inside doGet for GetLoginID.getFallbackPrincipal: "+PrincipalAccessor.getFallbackPrincipal()); */
		boolean debug = true;
		try{
            String userName="", passWord="", wsURL="", aggregatorID="";
            DestinationOptions options = DestinationOptions.builder().augmentBuilder(ScpCfDestinationOptionsAugmenter
                    .augmenter().tokenExchangeStrategy(ScpCfDestinationTokenExchangeStrategy.LOOKUP_ONLY)).build();
            Try<Destination> destinationAccessor = DestinationAccessor.getLoader()
                    .tryGetDestination("CPIConnect_clone", options);
            Destination cpiConfig = destinationAccessor.get(); 

            /* Context ctx = new InitialContext();
            ConnectivityConfiguration configuration = (ConnectivityConfiguration) ctx.lookup("java:comp/env/connectivityConfiguration");
            DestinationConfiguration cpiConfig = configuration.getConfiguration("CPIConnect_clone"); */
            ...
            /* userName = cpiConfig.getProperty("User");
            passWord = cpiConfig.getProperty("Password");
            wsURL = cpiConfig.getProperty("URL");
            aggregatorID=cpiConfig.getProperty("AggregatorID"); */
			...
			response.getWriter().println("Exception in GST Verification: "+buffer.toString());
		}
