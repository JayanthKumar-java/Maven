...
public class ESignContractCALCompaniesRenewal extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String inputPayload = "", wsURL = "", userName = "", passWord = "", userpass = "",
				eSignCntCALCompRenewalEndpnt = "", eSignCntCALCompRenewalres = "";
		boolean debug = false;
		...
		try {
			...
			if (inputPayload != null && !inputPayload.equalsIgnoreCase("")) {
				...
				if (wsURL != null && !wsURL.equalsIgnoreCase("")) {
					...
					eSignCntCALCompRenewalEndpnt = properties.getProperty("eSignContractCALCompaniesRenewal");
					wsURL = wsURL.concat(eSignCntCALCompRenewalEndpnt);
					if (debug) {
						response.getWriter().println("ESignContractCALCompaniesRenewal Url: " + wsURL);
					}
					...
					if ((resCode / 100) == 2 || (resCode / 100) == 3) {
						...
						eSignCntCALCompRenewalres = sb.toString();
						JsonParser jsonParser = new JsonParser();
						JsonObject responseJson = (JsonObject) jsonParser.parse(eSignCntCALCompRenewalres);
						...
						returnObj.addProperty("message", "responseCode:"+responseCode+"ResponseMessage:"+responseMessage);
						if (debug)
