...
public class Authorize extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean isHttpReqSuccess = false;
		...
		String idType="", idValue="", newReqRefNo="",inputCPGuid="", cpType="", pgGuid="", pgProvider="", aggregatorID="", pgID="", oneTimePwd="", configurationValues="", tncAgreed="";
		...
		try{
			...
			if(inputPayload.has("OTP") && inputPayload.get("OTP").getAsString().trim().length() > 0){
				oneTimePwd = inputPayload.get("OTP").getAsString().trim();
				boolean lengthOk = true;
				if(oneTimePwd.length() != 6){
					lengthOk = false;
				}
				
				if(! lengthOk){
					if(!validValueCheckMsg.equalsIgnoreCase("Received Invalid Values For: ")){
						validValueCheckMsg = validValueCheckMsg+", OTP: Expected 6 characters";
					}else{
						validValueCheckMsg = validValueCheckMsg+" OTP: Expected 6 characters";
					}
				}
				...
				if(! mandatoryCheckMsg.equalsIgnoreCase("Mandatory inputs missing: ")){
					mandatoryCheckMsg = mandatoryCheckMsg+", OTP";
				}else{
					mandatoryCheckMsg = mandatoryCheckMsg+"OTP";
				}
			}
			
			if(inputPayload.has("TnCAgreed") && inputPayload.get("TnCAgreed").getAsString().trim().length() > 0){
				tncAgreed = inputPayload.get("TnCAgreed").getAsString();
			}else{
				tncAgreed = "Y";
			}
			...
			if(debug){
				...
				response.getWriter().println("tncAgreed: "+tncAgreed);
				response.getWriter().println("pgProvider: "+pgProvider);
			}
			
			if(mandatoryCheckPass){
				String customerFilter ="", loginMethod="", destURL="", sessionID="", customerService="", service="", postalCode="";
					...
					if(userCustomerObj.has("Status") && userCustomerObj.get("Status").getAsString().equalsIgnoreCase("000001")){
						for(int i=0 ; i<userCustomerObj.get("d").getAsJsonObject().get("results").getAsJsonArray().size() ; i++){
							...
							try{
								...
								postalCode = userCustomerObj.getAsJsonObject("d").getAsJsonArray("results").get(i).getAsJsonObject().get("PostalCode").getAsString();
...
//							formattedStr = customerNo;
					...
					String transactionType="", transactionAmt="", cardNo="", mobileNo="", lastReqRefNo="", trackID="", bflDealerCode="", 
							dealerValKey="", ipAddress="", dealID="", cancelTxnType="", cancelRRN="";
					if(! isRequestFromCloud){
						if(arePrechecksSuccess){
							transactionType = properties.getProperty("AuthTxnType");
							if(idType.equalsIgnoreCase("MOBILE")){
								mobileNo = idValue;
								cardNo = "";
							}else if(idType.equalsIgnoreCase("CARD")){
							...
							transactionAmt = paymentTxnObj.get("Payment").getAsJsonObject().get("PaymentAmount").getAsString();
							...
							String pgName="", ivText="", txnKey="", schemeCode="", txnURL="", tenure="";
							...
							if(debug){
								...
								response.getWriter().println("transactionAmt: "+transactionAmt);
								response.getWriter().println("tncAgreed: "+tncAgreed);
								...
								response.getWriter().println("oneTimePwd: "+oneTimePwd);
							}
							
							//Prepare payload for BFL
							...
							bflPayload.addProperty("LOANAMT", transactionAmt);
							bflPayload.addProperty("TNC", tncAgreed);
							
							if(mobileNo.trim().length() > 0){
								bflPayload.addProperty("MOBNO", mobileNo);
								bflPayload.addProperty("CN", "");
							}else{
								bflPayload.addProperty("CN", cardNo);
								bflPayload.addProperty("MOBNO", "");
							}
							
							bflPayload.addProperty("OTP", oneTimePwd);
							bflPayload.addProperty("TENURE", tenure);
							bflPayload.addProperty("PINCODE", postalCode);
							bflPayload.addProperty("RRN", newReqRefNo);
							bflPayload.addProperty("DLRVKEY", dealerValKey);
							bflPayload.addProperty("SCHEMID", schemeCode);
							bflPayload.addProperty("REQFTXT2", pgGuid);
							
							if(debug)
								response.getWriter().println("bflPayload for Authorize: "+bflPayload);
							
							String encodedValue="", sealValue="", encryptedResponse="", decodedValue="";
							encodedValue = CommonUtils.AES_Encode(bflPayload.toString(), txnKey, ivText);
							if(debug)
							...
							if(csrfToken.equalsIgnoreCase("Failure") || csrfToken.contains("Exception")){
								...
								if(updateTxnRes.equalsIgnoreCase("204")){
									try{
										...
										responseObj.addProperty("Exception", e.getClass()+"|"+e.getCause()+"|"+buffer.toString());
										responseObj.addProperty("PGTransactionID", newReqRefNo);
										response.getWriter().println(responseObj);
									}
									...
									if(isHttpReqSuccess){
										if(encryptedResponse != null){
								        	...
								        	decodedValue = CommonUtils.AES_Decode(encryptedResponse.substring(0,encryptedResponse.lastIndexOf("|")),txnKey, ivText);
								        	if(debug)
											...
											if(bflResponseJson.get("RSPCODE").getAsString().equalsIgnoreCase("00")
												...
												responseObject.addProperty("PF", bflResponseJson.get("PF").getAsString());
												responseObject.addProperty("Charges", bflResponseJson.get("CHARGES").getAsString());
												responseObject.addProperty("AdvEMIAmt", bflResponseJson.get("ADVEMIAMT").getAsString());
												responseObject.addProperty("TotalDownPymnt", bflResponseJson.get("DOWNPAYMT").getAsString());
												responseObject.addProperty("TransactionAmt", bflResponseJson.get("LOANAMT").getAsString());
												responseObject.addProperty("Tenure", bflResponseJson.get("TENURE").getAsString());
												responseObject.addProperty("AdvEMITenure", bflResponseJson.get("ADVEMITENURE").getAsString());
												responseObject.addProperty("FirstDueDate", bflResponseJson.get("FDD").getAsString());
												responseObject.addProperty("EmiPerMonth", bflResponseJson.get("EMIPERMONTH").getAsString());
												responseObject.addProperty("PGPaymentGUID", pgGuid);
												responseObject.addProperty("ResponseDtt", bflResponseJson.get("RESDTT").getAsString());
												responseObject.addProperty("BankReferenceNo", bflResponseJson.get("DEALID").getAsString());
												responseObject.addProperty("PGTransactionID", newReqRefNo);
												response.getWriter().println(responseObject);
											}/*DO NOT DELETE THE BLOCK. TO BE USED IN FUTURE
											else if(! bflResponseJson.get("RSPCODE").getAsString().equalsIgnoreCase("35")){
												//Cancel the request
												JsonObject cancelResponse = new JsonObject();
												dealID = bflResponseJson.get("DEALID").getAsString();
												cancelTxnType=properties.getProperty("CancelTxnType");
												
												cancelRRN="";
												String sub=newReqRefNo.substring(newReqRefNo.length()-3, newReqRefNo.length());
												if(sub.startsWith("X")){
													String formattedStr="";
													try{
														int i = Integer.parseInt(sub.substring(sub.length()-2, sub.length()));
														...
														cancelRRN = pgGuid+formattedStr;
													}catch (NumberFormatException e) {
														cancelRRN = pgGuid+"X01";
													}
												}else{
													cancelRRN = pgGuid+"X01";
												}
												...
												if(debug){
													response.getWriter().println("dealID: "+dealID);
													response.getWriter().println("cancelTxnType: "+cancelTxnType);
													response.getWriter().println("AuthReqRefNo: "+newReqRefNo);
													response.getWriter().println("cancelRRN: "+cancelRRN);
													response.getWriter().println("trackID: "+trackID);
													response.getWriter().println("transactionAmt: "+transactionAmt);
												}
												
												//Payload for cancel request
												JsonObject cancelBflPayload = new JsonObject();
												cancelBflPayload.addProperty("TXNTYPE", cancelTxnType);
												cancelBflPayload.addProperty("DLRCODE", bflDealerCode);
												cancelBflPayload.addProperty("DEALID", dealID);
												cancelBflPayload.addProperty("ORDERNO", trackID);
												cancelBflPayload.addProperty("RRN", cancelRRN);
												cancelBflPayload.addProperty("LOANAMT", transactionAmt);
												cancelBflPayload.addProperty("DLRVKEY", dealerValKey);
												
												if(debug)
													response.getWriter().println("cancelBflPayload: "+cancelBflPayload);
												
												cancelResponse = processRequest(request, response, cancelBflPayload, txnURL, txnKey, ivText, debug);
												if(debug)
													response.getWriter().println("cancelResponse: "+cancelResponse);
												if(cancelResponse.has("Status")){
													response.getWriter().println(cancelResponse);
												}else{
													if(cancelResponse.get("RSPCODE").getAsString().equalsIgnoreCase("00")
														|| cancelResponse.get("RSPCODE").getAsString().equalsIgnoreCase("0")){
														responseObject.addProperty("Status", "000002");
														responseObject.addProperty("Message", cancelResponse.get("ERRDESC").getAsString());
														responseObject.addProperty("ErrorCode", cancelResponse.get("RSPCODE").getAsString());
														//Success response from Cancel means transaction cancelled/failed succesfully
														responseObject.addProperty("TransactionStatus", "03");
														if(bflResponseJson.has("PF"))
															responseObject.addProperty("PF", bflResponseJson.get("PF").getAsString());
														else
															responseObject.addProperty("PF", "0");
														
														responseObject.addProperty("Charges", bflResponseJson.get("CHARGES").getAsString());
														responseObject.addProperty("AdvEMIAmt", bflResponseJson.get("ADVEMIAMT").getAsString());
														if(bflResponseJson.has("DOWNPAYMT"))
															responseObject.addProperty("TotalDownPymnt", bflResponseJson.get("DOWNPAYMT").getAsString());
														else
															responseObject.addProperty("TotalDownPymnt", bflResponseJson.get("DOWNPAYMT").getAsString());
														
														if(bflResponseJson.has("LOANAMT"))
															responseObject.addProperty("TransactionAmt", bflResponseJson.get("LOANAMT").getAsString());
														else
															responseObject.addProperty("TransactionAmt", "0");
														
														if(bflResponseJson.has("TENURE"))
															responseObject.addProperty("Tenure", bflResponseJson.get("TENURE").getAsString());
														else
															responseObject.addProperty("Tenure", "0");
														
														if(bflResponseJson.has("ADVEMITENURE"))
															responseObject.addProperty("AdvEMITenure", bflResponseJson.get("ADVEMITENURE").getAsString());
														else
															responseObject.addProperty("AdvEMITenure", "0");
														
														if(bflResponseJson.has("FDD"))
															responseObject.addProperty("FirstDueDate", bflResponseJson.get("FDD").getAsString());
														else
															responseObject.addProperty("FirstDueDate", "");
															
														if(bflResponseJson.has("EMIPERMONTH"))
															responseObject.addProperty("EmiPerMonth", bflResponseJson.get("EMIPERMONTH").getAsString());
														else
															responseObject.addProperty("EmiPerMonth", "0");
														
														responseObject.addProperty("PGPaymentGUID", pgGuid);
														responseObject.addProperty("ResponseDtt", cancelResponse.get("RESDTT").getAsString());
														responseObject.addProperty("BankReferenceNo", bflResponseJson.get("DEALID").getAsString());
														responseObject.addProperty("PGTransactionID", cancelRRN);
														response.getWriter().println(responseObject);
													}else{
														responseObject.addProperty("Status", "000002");
														responseObject.addProperty("Message", cancelResponse.get("ERRDESC").getAsString());
														responseObject.addProperty("ErrorCode", cancelResponse.get("RSPCODE").getAsString());
														responseObject.addProperty("TransactionStatus", cancelResponse.get("RSPCODE").getAsString());
														if(bflResponseJson.has("PF"))
															responseObject.addProperty("PF", bflResponseJson.get("PF").getAsString());
														else
															responseObject.addProperty("PF", "0");
												...
												if(bflResponseJson.has("ERRDESC"))
													...
													responseObject.addProperty("Message", "Internal - ERRDESC field from API is not available in response");
												
												if(bflResponseJson.has("RSPCODE")){
													responseObject.addProperty("ErrorCode", bflResponseJson.get("RSPCODE").getAsString());
													responseObject.addProperty("TransactionStatus", bflResponseJson.get("RSPCODE").getAsString());
												}else{
													responseObject.addProperty("ErrorCode", "99");
													responseObject.addProperty("TransactionStatus", "99");
												}
												...
												if(bflResponseJson.has("CHARGES"))
													responseObject.addProperty("Charges", bflResponseJson.get("CHARGES").getAsString());
												else
													responseObject.addProperty("Charges", "0");
												
												if(bflResponseJson.has("ADVEMIAMT"))
													responseObject.addProperty("AdvEMIAmt", bflResponseJson.get("ADVEMIAMT").getAsString());
												else
													responseObject.addProperty("AdvEMIAmt", "");
													
												if(bflResponseJson.has("DOWNPAYMT"))
													responseObject.addProperty("TotalDownPymnt", bflResponseJson.get("DOWNPAYMT").getAsString());
												else
													responseObject.addProperty("TotalDownPymnt", "0");
												...
												if(bflResponseJson.has("RESDTT"))
													responseObject.addProperty("ResponseDtt", bflResponseJson.get("RESDTT").getAsString());
												else
													responseObject.addProperty("ResponseDtt", "");
												
												if(bflResponseJson.has("DEALID"))
													responseObject.addProperty("BankReferenceNo", bflResponseJson.get("DEALID").getAsString());
												else
													responseObject.addProperty("BankReferenceNo", "");
...
//						response.getWriter().println("nodeList merchantCodeList: "+merchantCodeList.item(i).getTextContent());
					...
					for(int i=0 ; i<pgCategoryList.getLength() ; i++)
					{
						if(PGID.equalsIgnoreCase(pdIDList.item(i).getTextContent()))
						{
							configurableValues = configurableValues	+"PGID="+pdIDList.item(i).getTextContent()
													...
													+"|"+"SchemeCode="+schemeCodeList.item(i).getTextContent()
													+"|"+"Tenure="+clientCodeList.item(i).getTextContent()
	...
	public JsonObject processRequest(HttpServletRequest request, HttpServletResponse response, JsonObject inputPayload, 
		String transactionURL, String encryptionKey, String ivText, boolean debug) throws IOException{
		String encodedValue="", sealValue="", encryptedResponse="", decodedValue="";
		JsonObject responseObject = new JsonObject();
		try{
			encodedValue = CommonUtils.AES_Encode(inputPayload.toString(), encryptionKey, ivText);
			if(debug)
				response.getWriter().println("processRequest.encodedValue: "+encodedValue);
			sealValue = CommonUtils.getSealValue(encodedValue+encryptionKey);
			if(debug)
				response.getWriter().println("processRequest.sealValue: "+sealValue);
			
			Client client=Client.create();
			WebResource webResource = client.resource(transactionURL);
			encryptedResponse = webResource.header("SealValue", sealValue).type("application/json").post(String.class, "\""+encodedValue+"\"");
			if(debug)
				response.getWriter().println("processRequest.encryptedResponse: "+encryptedResponse);
			
			if(encryptedResponse != null){
				encryptedResponse = encryptedResponse.replace("\"", "");
				if(debug){
					response.getWriter().println("processRequest.encryptedResponse after removing double quotes: "+encryptedResponse);
				    response.getWriter().println("processRequest.input for decryption: "+encryptedResponse.substring(0, encryptedResponse.lastIndexOf("|")));
				}
	        	
	        	decodedValue = CommonUtils.AES_Decode(encryptedResponse.substring(0,encryptedResponse.lastIndexOf("|")),encryptionKey, ivText);
	        	if(debug)
	        		response.getWriter().println("processRequest.decodedValue after calling POST: "+decodedValue);
	        	
	        	JsonParser bflResponseParser = new JsonParser();
	        	responseObject = (JsonObject)bflResponseParser.parse(decodedValue);
	        	
	        	return responseObject;
			}else{
				if(debug)
		    		response.getWriter().println("processRequest.Getting Empty response from ATOS service");
		    	
				responseObject.addProperty("Status", "000002");
				responseObject.addProperty("Message", "Response not received from: "+transactionURL);
				responseObject.addProperty("ErrorCode", "J001");
				responseObject.addProperty("PGTransactionID", inputPayload.get("RRN").getAsString());
				return responseObject;
			}
		}catch (Exception e) {
			StackTraceElement element[] = e.getStackTrace();
			...
			if(debug){
				response.getWriter().println("processRequest-Exception Stack Trace: "+buffer.toString());
			}
			
			responseObject.addProperty("Status", "000002");
			responseObject.addProperty("Message", e.getClass()+"-"+e.getCause()+"-"+e.getMessage());
			responseObject.addProperty("ErrorCode", "JE001");
			responseObject.addProperty("Exception", e.getClass()+"|"+e.getCause()+"|"+buffer.toString());
			responseObject.addProperty("PGTransactionID", inputPayload.get("RRN").getAsString());
			return responseObject;
//			response.getWriter().println(responseObj);
		}
